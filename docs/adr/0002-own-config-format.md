...
tnl's config is its own design: tunnels keyed by name under `tunnels:`, each with `host`, optional `user`/`identity_file`/`enabled`/`reclaim`, and a `ports:` list of `local:remote` specs — or `local:desthost:remote` to forward through the ssh host — with an optional `label: <spec>` pair form for display names. It lives at `~/.tnlrc.yaml` (overridable with `-c`).
