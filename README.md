...
- `tnl-linux-amd64` — statically linked release binary (`CGO_ENABLED=0`, `-trimpath`) with the tag version (leading `v` stripped) baked into `internal/version.Version`
- `tnl-linux-amd64.intoto.jsonl` — signed SLSA provenance generated by the builder

A `workflow_dispatch` run builds the same artifact with a `dev-<commit>` version but does not upload release assets, so releases are only ever created from `v*` tags.

The workflow needs GitHub Actions to mint OIDC tokens (keyless signing) and the job permissions `id-token: write`, `contents: write`, and `actions: read`; make sure repository or organization settings don't restrict them. Create release tags from the commit you intend to ship. Consumers can verify the attached provenance with the SLSA verifier; always verify before trusting an artifact.
