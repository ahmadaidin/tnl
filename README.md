...
The workflow needs GitHub Actions to mint OIDC tokens (keyless signing). The builder uses `id-token: write`, `contents: read`, and `actions: read`; only the final publish job uses `contents: write`. Make sure repository or organization settings don't restrict these permissions. Create release tags from the commit you intend to ship. Consumers can verify the attached provenance with the SLSA verifier; always verify before trusting an artifact.
