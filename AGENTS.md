...
- `internal/sshsetup` — `Provisioner{SSHBin, KeygenBin, HomeDir, EffectiveConfig, Run, Prompt, Out, Err}` with `Provision(ctx, tunnel, options) Result`; injectable exec + paths + HOME (zero value = real defaults: system `ssh`/`ssh-keygen`, `ssh -G <host>` for effective identity/user resolution). Generates a keypair, inserts `IdentityFile` into `~/.ssh/config` before the first matching `Host`/`Match` block (or appends), pushes the pubkey via an idempotent `grep -qFx`-then-append remote command with `-o StrictHostKeyChecking=accept-new`, and rolls back (key + config bytes) on failure. One Identity per (host, effective-user) pair; a host with an existing Identity is skipped.
...
Keep the verbs stable: `tnl [names]` (foreground), `-d/--detach`, `status`, `start [names]`, `stop` (daemon) vs `stop <name>`, `restart <name>`, `setup [name]`, `install`, `uninstall`, `version`, `-c/--config`, `-h/--help`. `tnl start <name>` auto-daemonizes when no daemon is running. `tnl setup` refuses to run while the daemon is running. A bare `tnl` while the daemon runs is an error.
