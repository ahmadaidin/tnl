...
func TestSpawnArgsDestHost(t *testing.T) {
	...
	want := strings.Fields("-N -L 3329:db.suteki.tech:3306 -o ServerAliveInterval=5 -o ServerAliveCountMax=2 -o ExitOnForwardFailure=yes -l aidin dev.civitas.id")
...
func TestSpawnArgsNoDestHost(t *testing.T) {
	...
	want := strings.Fields("-N -L 3000:localhost:3000 -o ServerAliveInterval=5 -o ServerAliveCountMax=2 -o ExitOnForwardFailure=yes example.com")
	if !reflect.DeepEqual(got, want) {
		t.Errorf("spawnArgs = %q, want %q", got, want)
	}
}
